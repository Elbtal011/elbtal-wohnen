...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Import completed - metadata restored to reconnect leads with existing Storage files
    console.log('Metadata import completed - leads should now show their existing documents')

    result.success = true
    result.message = `Import completed: ${result.details.contactRequests.inserted + result.details.contactRequests.updated} contacts, ${result.details.leadDocuments.inserted} lead docs, ${result.details.userDocuments.inserted} user docs restored`
