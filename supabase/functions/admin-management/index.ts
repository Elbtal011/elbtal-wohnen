...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_lead_documents': {
        const { contact_request_id } = data;
        if (!contact_request_id) {
          return new Response(
            JSON.stringify({ error: 'contact_request_id is required' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
        ...
        for (const doc of docs || []) {
          try {
            ...
            withUrls.push({ ...doc, signed_url: urlData?.signedUrl || null });
          } catch (e) {
            withUrls.push({ ...doc, signed_url: null });
          }
        }
        return new Response(
          JSON.stringify({ documents: withUrls }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      case 'upload_lead_document': {
        const { contact_request_id, file_name, content_type, file_base64 } = data;
        if (!contact_request_id || !file_name || !content_type || !file_base64) {
          return new Response(
        ...
        const ext = (file_name as string).split('.').pop();
        const path = `${contact_request_id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;

        // Decode base64
        const binary = atob(file_base64 as string);
        ...
        const { error: uploadErr } = await supabase.storage
          .from('lead-documents')
          .upload(path, blob, { contentType: content_type });
        if (uploadErr) throw uploadErr;

        const file_size = bytes.length;
        const document_type = content_type === 'application/pdf' ? 'PDF' : (content_type.startsWith('image/') ? 'Bild' : 'Dokument');
        ...
        return new Response(
          JSON.stringify({ success: true, document: inserted }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      case 'delete_lead_document': {
        ...
        if (!id || !file_path) {
          return new Response(
            JSON.stringify({ error: 'id and file_path are required' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
